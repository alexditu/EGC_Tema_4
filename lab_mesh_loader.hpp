...
namespace lab{
	...
	// incarca cele 3 fete
	void loadObj(const std::string &filename1, const std::string &filename2, const std::string &filename3,
		...
		// concatenare valori in vertices1
		unsigned int vsize, isize;
		vsize = vertices1.size();
		isize = indices1.size();

		for (int i = 0; i < vsize; i++) {
			vertices1.push_back(vertices2[i]);
		}
		for (int i = 0; i < vsize; i++) {
			vertices1.push_back(vertices3[i]);
		}
		...
		glGenBuffers(1,&gl_vertex_buffer_object);
		glBindBuffer(GL_ARRAY_BUFFER, gl_vertex_buffer_object);
		glBufferData(GL_ARRAY_BUFFER, 3*vsize*sizeof(VertexFormat), &vertices1[0], GL_STATIC_DRAW);

		//index buffer object -> un obiect in care tinem indecsii
		glGenBuffers(1,&gl_index_buffer_object);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gl_index_buffer_object);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, isize*sizeof(unsigned int), &indices1[0], GL_STATIC_DRAW);
		...
		glVertexAttribPointer(3,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(vsize * sizeof(VertexFormat) + 0));						//trimite pozitii pe pipe 0
		glEnableVertexAttribArray(4);
		glVertexAttribPointer(4,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(vsize * sizeof(VertexFormat) + sizeof(float)*3));		//trimite normale pe pipe 1
		glEnableVertexAttribArray(5);
		glVertexAttribPointer(5,2,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(vsize * sizeof(VertexFormat) + 2*sizeof(float)*3));	//trimite texcoords pe pipe 2

		// fata 3
		glEnableVertexAttribArray(6);
		glVertexAttribPointer(6,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2 * vsize * sizeof(VertexFormat) + 0));						//trimite pozitii pe pipe 0
		glEnableVertexAttribArray(7);
		glVertexAttribPointer(7,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2 * vsize * sizeof(VertexFormat) + sizeof(float)*3));		//trimite normale pe pipe 1
		glEnableVertexAttribArray(8);
		glVertexAttribPointer(8,2,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2 * vsize * sizeof(VertexFormat) + 2*sizeof(float)*3));	//trimite texcoords pe pipe 2
